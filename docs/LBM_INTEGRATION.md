...
The weights, benchmarks, and geometry diagnostics in this document were produced on an **Intel® Xeon® 6737P (Granite Rapids)** dual-socket system (128 logical CPUs, 1 TB DDR5-6400, AMX-BF16 / AMX-INT8) made available to Dotsin.ai by **Intel Corporation**. The platform delivered the measurable headroom this repository is built around — two-pass fine-tune in ~21 h total wall-clock, OpenVINO BF16 inference at **4.6 – 6.2× over PyTorch FP32**, single-query p50 at **9 – 10 ms** (a 113 – 145× reduction vs PyTorch FP32), and a production NNCF INT8 serving preset that reaches **135 K – 182 K TPS** with ≥ 99.4 % cosine fidelity. The 288 MiB L3 and 1 TB DDR5-6400 memory pool leave clear headroom for longer context, larger batches, and larger successor encoders on the same node. That envelope is what currently carries Dotsin's **early production embedding traffic for the secure data hub** on this single 2-socket node, at the latency our LBM information-stream assembly requires. **For groups reproducing or extending this work,** a Granite Rapids-class node with this AMX / OpenVINO / NNCF / oneDNN configuration is the setup that reproduces the same CPU and DRAM utilization profile end-to-end.

The encoders described here are the **open public version** of one layer of Dotsin.ai's larger system; Dotsin maintains a deeper-tuned proprietary causal-similarity embedding stack trained against the full BODHI ontology and additional behavioural corpora, and the LBM service and proprietary LBM graph remain closed. This repository is published so the open biomedical-NLP community can evaluate the causal-axis research direction and build on it. Full per-metric breakdown and credits: [`../ACKNOWLEDGEMENTS.md`](../ACKNOWLEDGEMENTS.md).
